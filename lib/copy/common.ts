...
export const common = {
  ...
  // На какой момент известна обстановка — на главной, в предупреждениях и на
  // карте. Время приходит из CMS (alerts.updated_at); если оно старше срока,
  // который выбрал администратор (settings.situation, по умолчанию сутки),
  // рядом появляется `outdated`.
  situation: {
    // «Обстановка на 09:42 (UTC+5)».
    asOf: "Обстановка на",
    outdated:
