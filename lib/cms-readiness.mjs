...
/**
 * Публичный адрес сайта обязателен для production-сборки.
 *
 * canonical, og:url, JSON-LD, sitemap и robots резолвятся из
 * NEXT_PUBLIC_SITE_URL (см. lib/seo.ts). Без него во все эти сигналы попадает
 * `http://localhost:3000` — сборка «успешна», а сайт для роботов сломан.
 * Понятная ошибка на старте сборки вместо тихой порчи метаданных.
 *
 * Побеги (осознанные, не по умолчанию):
 *  - `CMS_BUILD_MODE=preview` — превью-сборки и так допускают резервный контент;
 *  - `SITE_URL_ALLOW_LOCAL=1` — локальная проверка production-артефакта.
 */
export function assertSiteUrlForBuild(environment = process.env) {
  const siteUrl = environment.NEXT_PUBLIC_SITE_URL;

  if (cmsBuildMode(environment) === "preview" || environment.SITE_URL_ALLOW_LOCAL === "1") {
    return;
  }
  ...
  const isLocal =
    ...
    /\.(local|test)$/i.test(hostname);

  if (isLocal) {
    throw new CmsReadinessError(
      "invalid_site_url",
      [
        "NEXT_PUBLIC_SITE_URL must be a public HTTPS origin for a production build",
        `(got: ${siteUrl || "<unset>"}; localhost would leak into canonical/og:url/sitemap).`,
        "Set NEXT_PUBLIC_SITE_URL=https://khf.tj (or pass SITE_URL_ALLOW_LOCAL=1 / CMS_BUILD_MODE=preview for a deliberate local artifact).",
      ].join(" "),
    );
  }
}
