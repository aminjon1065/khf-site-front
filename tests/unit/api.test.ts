...
describe("lib/api", () => {
  ...
  describe("fetchNews / buildUrl", () => {
    it("converts the tj URL locale to the tg API locale in the query string", async () => {
      fetchMock.mockResolvedValueOnce(
        jsonResponse({ data: [], meta: { total: 0, per_page: 20, current_page: 1, last_page: 1 } }),
      );
    ...
    it("maps perPage to the per_page query parameter", async () => {
      fetchMock.mockResolvedValueOnce(
        jsonResponse({ data: [], meta: { total: 0, per_page: 12, current_page: 1, last_page: 1 } }),
      );
  ...
  describe("fetchNewsItem", () => {
    ...
    it("returns the item's data on success", async () => {
      const item = { slug: "some-slug", title: "Заголовок", category: "Новости", date: "16 июля 2026" };
